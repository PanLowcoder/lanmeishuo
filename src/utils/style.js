 ...
 * @param {*} showTabBar
 */
export function getWindowHeight(showTabBar = false, showNav = true, subHeight = 0) {
  const info = Taro.getSystemInfoSync()
  const {windowHeight, statusBarHeight, titleBarHeight} = info
  const tabBarHeight = showTabBar ? TAB_BAR_HEIGHT : 0;
  const navHeight = showNav ? NAVIGATOR_HEIGHT : 0;
  const allHeight = tabBarHeight + navHeight + subHeight;

  if (process.env.TARO_ENV === 'rn') {
    return windowHeight - statusBarHeight - NAVIGATOR_HEIGHT - allHeight
  }

  if (process.env.TARO_ENV === 'h5') {
    return `${windowHeight - allHeight}`
  }
